...
const EditProfile = () => {
  ...
  const { data: getOneUser } = useGet<IUser | undefined>(
    `/users/getUserId/${user?.id}`
  );

  useEffect(() => {
    setUsername(getOneUser?.username);
    setFirstName(getOneUser?.firstName);
    setLastName(getOneUser?.lastName);
    setCity(getOneUser?.city);
    setCountry(getOneUser?.country);
    setBio(getOneUser?.bio);
    setImage(getOneUser?.image);
  }, [getOneUser]);
